...
class StarModel(BaseModel):
    ...
    def _build_vertices(self):
        ...
        for i in range(self.num_points):
            ...
            if i % 2 == 0:
                x = self.long_radius * np.cos(angle)
                y = self.long_radius * np.sin(angle)
            else:
                x = self.short_radius * np.cos(angle)
                y = self.short_radius * np.sin(angle)
    ...
    def _build_texcoords(self):
        if self.render_mode == "Texture":
            ...
            for vertex in self.vertices:
                u = 0.5 + 0.5 * (vertex[0] / self.long_radius)
                v = 0.5 + 0.5 * (vertex[1] / self.long_radius)
                uvs.append([u, v])
